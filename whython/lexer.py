...
class Lexer:
        ...
        while self.current_char is not None:
            if self.current_char in ' \t':
                ...
                for allowed_token in allowed_tokens:
                    if allowed_token == self.current_char:
                        tokens.append(Token(allowed_tokens[allowed_token], pos_start=self.pos))
                        self.advance()
                        found_token = True
                if not found_token:
    ...
    def make_string(self):
        ...
        while self.current_char is not None and (self.current_char != '"' or escape_character):
            if escape_character:
                final_string += escape_characters.get(self.current_char, self.current_char)
                escape_character = False
            else:
                if self.current_char == '\\':
